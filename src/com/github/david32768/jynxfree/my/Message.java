...
public enum Message implements JynxMessage {
    ...
    M92(BLANK,"do not print detailed differences"),
    ...
    M358(ERROR,"%s is a %s key word and cannot be a Java Id"),
    M359(ERROR,"start = %s cannot be a preview version or null"),
    M360(ERROR,"preview = %s must be a preview version(s)"),
    M361(ERROR,"all preview versions %s must be less than start = %s"),
    M363(ERROR,"start = %s must be less than end = %s"),
    M364(ERROR,"deprecate = %s must be in range [start,end]: start = %s end = %s"),
